...
CREATE TABLE IF NOT EXISTS `ksqdigital`.`cliente` (
  ...
  `clientecol` VARCHAR(45) NULL DEFAULT '1',
  PRIMARY KEY (`Cliid`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `ksqdigital`.`ordenproduccion` (
  ...
  `Ordenprod_created_at` TIMESTAMP GENERATED ALWAYS AS (CURRENT_TIMESTAMP) VIRTUAL COMMENT 'Registra el tiempo de creacion de la tupla',
  ...
  `ordenproduccioncol` VARCHAR(45) NULL,
  ...
  UNIQUE INDEX `OrdPAsignada_UNIQUE` (`OrdPAsignada` ASC),
...
CREATE TABLE IF NOT EXISTS `ksqdigital`.`proins` (
  `Productos_ProCodigo` INT(11) NOT NULL AUTO_INCREMENT,
  `Insumos_InsCodigo` VARCHAR(5) CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci' NOT NULL,
